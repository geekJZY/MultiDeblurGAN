...
sys.argv += ['--dataroot', '/scratch/user/jiangziyu/train/',
             ...
             '--fineSize', '256','--batchSize','1','--name','fullModel']
...
optimizer = torch.optim.Adam(itertools.chain(filter(lambda p: p.requires_grad, netG_frozen_deblur.parameters()),filter(lambda p: p.requires_grad, netG_frozen_blur.parameters())), lr=learning_rate)
...
for epoch in range(num_epoch):
    for i, data in enumerate(dataloader):
        ...
        loss_unsupervise.backward()
        optimizer.step()
        
        if (i + 1) % 20 == 0:
            print("(epoch %d itr %d) unsupervised loss is %f"% (epoch, i+1, loss_unsupervise.data[0]))
            print("(epoch %d itr %d) unsupervised loss is %f"% (epoch, i+1, loss_unsupervise.data[0]), file=open("outputFullModelMultiUnsupervised.txt", "a"))
         
        
    if epoch%10 ==0:    ##save deblur once every 10 epochs
