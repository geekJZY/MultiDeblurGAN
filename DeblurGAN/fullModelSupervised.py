...
sys.argv += ['--dataroot', '/scratch/user/jiangziyu/train/',
             ...
             '--fineSize', '256','--batchSize','8','--name','fullModelSupervised']
...
netG_frozen_deblur= freeze_single_input(netG_deblur, num_layers_frozen=21)
netG_frozen_blur= freeze_multi_input(netG_blur, num_layers_frozen=21)
...
for epoch in range(num_epoch):
    for i, data in enumerate(dataloader):
        ...
        netG_all_freeze = netG_frozen_deblur
        netG_all_freeze = freeze_single_input(netG_all_freeze, num_layers_frozen=50)
        # freeze all the layers for finetuning

        optimizer.zero_grad()

        #forward loss part 
        deblur_out0 = netG_all_freeze.forward(images0)
        deblur_out1 = netG_frozen_deblur.forward(images1)
        deblur_out2 = netG_all_freeze.forward(images2)
        ...
        loss_perceptual = contentLoss.get_loss(deblur_out1, labels)
        loss = loss_unsupervise + loss_perceptual*0.004
        #backward loss part
        ...
        if (i + 1) % 20 == 0:
            print("(epoch %d itr %d), unsupervised loss is %f, perceptual loss is %f, loss is %f"% (epoch, i+1, loss_unsupervise.data[0], loss_perceptual.data[0], loss.data[0]))
            print("(epoch %d itr %d), unsupervised loss is %f, perceptual loss is %f, loss is %f"% (epoch, i+1, loss_unsupervise.data[0], loss_perceptual.data[0], loss.data[0]), file=open("outputFullModelMultiSupervised.txt", "a"))
